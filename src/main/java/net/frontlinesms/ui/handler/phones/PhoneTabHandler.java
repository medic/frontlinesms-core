...
//> STATIC HELPER METHODS
	...
	private static String getServiceStatusAsString(FrontlineMessagingService service) {
		...
		if (service instanceof MmsEmailService && service.getStatus().equals(MmsEmailServiceStatus.READY)) {
			...
			if (lastChecked != null) {
				statusString += " - " + InternationalisationUtils.getI18nString(I18N_EMAIL_LAST_CHECKED, InternationalisationUtils.getDatetimeFormat().format(new Date (lastChecked).getTime()));
			}
		}
